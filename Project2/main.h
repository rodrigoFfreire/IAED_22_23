...
typedef struct {
    ...
    Stop* stops;
    Link* links;
    int line_count, stop_count, link_count;
...
void update_arrays_line(Network* system, int id, Link* new, int new_count);
...
void skip_over_link(Network* system, int* i, int* line_link_i, int n_links,
                        Link** new_links, int* new_link_count, int stop_id);

void update_arrays_stop(Network* system, int id, Link* new, int new_count);
